...
export default class ComponentTreeScript extends FIRENEF.Script {
    ...
    static type = "ComponentTreeScript";
    ...
    refreshComponents() {
        ...
        if (this.storedItemUi.length <= 0) return;
    ...
    newComponent(className, classObject) {
        ...
        if (this.startModule == "project") {
            if (!this.editor.projectModules["project"].updater) this.editor.projectModules["project"].updater = [];
            this.editor.projectModules["project"].updater.push(newComponent);
        } else {
            if (!this.editor.projectModules[this.startModule].children) this.editor.projectModules[this.startModule].children = [];
            this.editor.projectModules[this.startModule].children.push(newComponent);
        }
    ...
    removeComponent(childIndex) {
        if (this.startModule == "project") {
            this.editor.projectModules["project"].updater.splice(childIndex, 1);
        } else {
            this.editor.projectModules[this.startModule].children.splice(childIndex, 1);
        }
    }
    ...
    start() {
        ...
        this.editor.fetchNonAsyncFilesAsText(
            [
                "./src/workspace/ui/html/panel/componentTreeItem.html",
                "./src/workspace/ui/css/panel/componentTreeItem.css",
                "./assets/svgs/arrowClosed.svg",
                "./assets/svgs/arrowOpen.svg",
                "./src/workspace/ui/css/panel/componentIcon.css"
            ],
            (data) => {
                this.storedItemUi = data;
                this.refreshComponents();
                this.editor.onProjectLoaded.push(() => this.refreshComponents());
            }
        );
    }
}